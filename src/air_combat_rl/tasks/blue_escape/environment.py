...
class BlueEscapeEnv:
    ...
    def reset(self, seed: int | None = None) -> tuple[np.ndarray, dict[str, object]]:
        self.world = copy.deepcopy(self._initial_world); self.held_action = HeldAction(); self.policy_steps = 0; self.last_outcome = Outcome.RUNNING; self.reward_model.reset(self.world.snapshot())
